getwd()
setwd("E:/Rcode");getwd()
a=read.table("1.txt",header=TRUE)
plot(a$a)
plot(density(a$a))
plot(a$a,a$b)
qqplot(a$a,a$b)

b=read.table("2.txt",header=TRUE)
plot(b$POS)
plot(density(b$POS))
plot(b$CHROM,b$POS)

c=read.table("3.vcf",header=TRUE)
plot(c$POS)
plot(density(c$POS))
plot(c$CHROM,c$POS)
#readxls
library(readxl)
cyp<- read_excel("E:/Rcode/stat-test.xlsx",sheet = "统计详细", col_names = TRUE)
View(cyp)
